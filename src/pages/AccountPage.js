...
const AccountPage = () => {
  ...
  const validate = (values) => {
    ...
    if (!values.type.trim()) errors.type = "Este campo es necesario";
  ...
  const loadTutor = () => {
    axios
      .get(`${process.env.REACT_APP_SERVER}/api/pets/${user.id}`)
      .then((res) => {
        const data = res.data;
        ...
        setValues({
          name: tutor.name,
          type: tutor.type,
          specie: tutor.specie ? tutor.specie : "",
          sex: tutor.sex ? tutor.sex : "",
          birthday: tutor.birthday ? tutor.birthday : "",
  ...
  const updateUser = () => {
    axios
      .put(`${process.env.REACT_APP_SERVER}/api/users/${user.id}`, values, {
        headers: {
  ...
  const createTutor = async () => {
    ...
    await axios
      .post(`${process.env.REACT_APP_SERVER}/api/pets/${user.id}`, dataPet, {
        headers: {
          "x-access-token": localStorage.getItem("tl-token"),
        },
      })
      .then((res) => {
        const data = res.data;
        localStorage.setItem("tl-tutor", data.dataPet._id);
        const msg = data.msg;
        alert(msg);
      })
      .catch((err) => console.error(err));
    ...
    await axios
      .put(`${process.env.REACT_APP_SERVER}/api/tutors/${tutor_id}`, img, {
        headers: {
          "x-access-token": localStorage.getItem("tl-token"),
    ...
    <section className="text-gray-600 body-font">
      <div className="container px-5 py-24 mx-auto flex flex-wrap items-center">
        <form
          ...
          className="lg:w-3/6 md:w-2/3 bg-gray-100 rounded-lg px-11 py-8 mx-auto md:ml-auto w-full mt-10 md:mt-0"
        >
